...
namespace CommandManager {
	export function init() {
		...
		registerCmd(
			new SlashCommandBuilder()
				.addStringOption((option) => option.setName('type').setDescription('the content type').addChoices(
			...
			(user, interaction) => {
				...
				Arrays.division(contents, 5).forEach(conts => {
					...
					conts.forEach(cont => embed.addFields({
						name: cont.localName(user),
						value: cont.description(user) + '\n\n' + (cont.details(user) || '')
					}));
					embeds.push(embed);
				});
				if (embeds.length <= 0) embeds.push(new EmbedBuilder().setDescription('< empty >'));
				//TODO: make page
				/*
				Manager.start({
				...
				interaction.editReply({
					embeds: [
						new EmbedBuilder()
							...
							.addFields({ name: 'GOAL', value: bundle.find(user.locale, 'bot.goal') })
					]
				});
			}
		);
