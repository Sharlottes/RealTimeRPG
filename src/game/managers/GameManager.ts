    ...
    constructor({ user, ...options }: ManagerConstructOptions & { user: User }) {
        ...
        this.mainEmbed = new EmbedBuilder()
            ...
            .setFields(
                {
                    name: this.user.name,
                    value: `${bundle.find(this.locale, 'health')}: ${Canvas.unicodeProgressBar(this.user.stats.health, this.user.stats.health_max, { showValue: true })}\n`
                        + `${bundle.find(this.locale, 'energy')}: ${Canvas.unicodeProgressBar(this.user.stats.energy, this.user.stats.energy_max, { showValue: true })}\n`
                },
            )
        ...
        this.user.on('alert', async (alert) => {
            ...
            if (alert.lifetime !== -1 && this.message) {
                const { embeds: [{ description: cache }] } = this.message;
