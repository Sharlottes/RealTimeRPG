...
export function onKaKaoMessage(msg: Message) {
  ...
  if(!kakaoSession.has(msg.room)) {
    listeners.push(c=>msg.replyText(`[${c.guild?.name} #${(c.channel as TextChannel).name}] ${c.author.username}: ${c}`).catch(e => console.log(e)));
    kakaoSession.set(msg.room, listeners);
  }
  for(const [channel, room] of links.entries()) {
    if(msg.room === room) channel.send(`[${msg.room}] ${msg.sender.name}: ${msg.content}`); 
  }
}
