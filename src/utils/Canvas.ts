...
export class Canvas {
  ...
  public static unicodeProgressBar(
    ...
    bar += [
      ...
      "\u2589",
      "\u2588",
    ][Mathf.clamp(Math.floor(per / 8), 0, 8)];
    if (options?.showValue) bar += `  (${value}/${max})`;
    if (options?.showPercent) bar += `  (${per * 100}%)`;

    return bar;
  }

  public static donutProgressBar(
    ...
    const mainStyle =
      typeof font === "string" ? "#F47C7C" : font.style || "#F47C7C";
    const mainFont =
      typeof font === "string" ? "40px Arial" : font.font || "40px Arial";

    const subText = typeof sideFont === "string" ? sideFont : sideFont.text;
    const subStyle =
      typeof sideFont === "string" ? "#F47C7C" : sideFont.style || "#F47C7C";
    const subFont =
      typeof sideFont === "string"
        ? "40px Arial"
        : sideFont.font || "40px Arial";
